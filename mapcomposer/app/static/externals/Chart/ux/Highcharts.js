 ...
 *
 * A much improved & ported from Ext4Js 3 Highchart adapter. 
 *
 * - Supports the latest Highcharts (3.0.0)
 * - Supports both Sencha Ext4Js 4 and Touch 2
 * - Supports Highcharts animations
 *
 * In order to use this extension, you are expected to know how to use Highcharts and Sencha products (Ext4Js 4 &amp; Touch 2). 
 *
 * # Configuring Highcharts Ext4ension
 ...
 *       }
 *     }
 * ## Step 3: Create Ext4Js Store and data mappings
 *
 * Then we create a Ext4Js Store object to map the data fields.
 *     @example
 *     Ext4.define('SampleData', {
 ...
 *     @example
 *     var chart = new Ext4.create('Chart.ux.Highcharts', {
 *                     ....
 *                 });
 ...
 */
Ext4.define("Chart.ux.Highcharts", {
    ...
    statics: {
        ...
        sencha: function() {
            if (Ext4.versions.extjs) {
                return {
                    product: 'e',
                    major: Ext4.versions.extjs.major,
                    minor: Ext4.versions.extjs.minor,
                    name: 'e' + Ext4.versions.extjs.major
                };
            }
            if (Ext4.versions.touch) {
                return {
                    product: 't',
                    major: Ext4.versions.touch.major,
                    minor: Ext4.versions.touch.minor,
                    name: 't' + Ext4.versions.touch.major
                };
            }
            return {
                product: null,
     ...
     * This method can be overrided with Ext4.emptyFn for product deployment
    ...
    /**
     * @cfg {Object} loadMask An {@link Ext4.LoadMask} config or true to mask the
     * chart while
     ...
     * and extension in specific sequence. First, set the {@link Ext4.data.Store#cfg-autoLoad} 
     * option to false, create the Highcharts component with the store, then call the 
     * {@link Ext4.data.Store#method-load} method. The *initAnimAfterLoad* defers creating 
     ...
     * is rendered. **Note**: Do not call initial {@link Ext4.data.Store#method-load} inside this handler, 
     * especially with *initAnimAfterLoad* set to true because {@link Ext4.data.Store#method-load} will
     * never be called as the chart is deferring to render waiting for the store data. Here is an example
     * of how this should be called. This 'this' keyword refers to the Highcharts Ext4Js component whereas
     * chart refers to the created Highcharts chart object
     *       items: [{
     *          xtype: 'highchart',
     *          listeners: {
     *              afterChartRendered: function(chart) {
     *                  // 'this' refers to the 'highchart' Ext4Js component
     *                  var size = this.getSize();
     *                  // Get the average value of the first series
     *                  var temp = 0;
     *                  Ext4.each(chart.series[0].data, function(data) {
     *                      temp += data;
     *                  });
     *                  temp = temp / chart.series[0].data.length;
     *                  Ext4.Msg.alert('Info', 'The average value is ' + temp);
     *              }
     *          },
     *          series:[ ... ],
     *          xField: 'month',
    ...
    constructor: function(config) {
        ...
        this.afterChartRendered && (this.afterChartRendered = Ext4.bind(this.afterChartRendered, this));
    ...
    removeAllSeries : function() {
        ...
        // there are any
        Ext4.each(_this.chartConfig.xAxis, function(xAxis) {
    ...
    afterRender : function() {
        ...
        // Ext4.applyIf causes problem in 4.1.x but works fine with
        // 4.0.x
        Ext4.apply(_this.chartConfig.chart, {
            renderTo : (this.statics().sencha.product == 't') ? this.element.dom : this.el.dom
        });

        Ext4.applyIf(_this.chartConfig, {
            xAxis : [{}]
        });
    ...
    updatexAxisData : function() {
        ...
        if(_this.xField && this.store) {
            ...
            else if (Ext4.isArray(_this.chartConfig.xAxis)) {
    ...
    bindStore : function(store, initial) {
        ...
        if (this.loadMask !== false) {
            if (this.loadMask === true) {
                this.loadMask = new Ext4.LoadMask({target:this,store:this.store});
            } else {
            ...
            for(var x = 0; x < items.length; x++) {
                ...
                for( i = 0; i < seriesCount; i++) {
                    ...
                    if (serie.dataIndex || serie.yField || 
                        ...
                        // data points
                        if (Ext4.isArray(record.data[serie.dataIndex])) {
                            Ext4.each(record.data[serie.dataIndex], function(dataPoint) {
                ...
                for( i = 0; i < seriesCount; i++) {
                    ...
                    if (_this.series[i].useTotals) {
                        ...
                        if (!_this.lineShift) {
                            ...
                            var xAxis = Ext4.isArray(this.chart.xAxis) ? this.chart.xAxis[0] : this.chart.xAxis;
