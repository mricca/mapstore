 ...
 *     }]
 */
Ext4.define('Chart.ux.Highcharts.Serie', {
     ...
     *
     * This can be useful with pointclick event when you need to use an Ext4.Component.
     *     pointclick:{
    ...
    resolveColor: function(colorField, record, dataPtIdx) {
        ...
        var color = null;
        if (colorField) {
            if (Ext4.isNumeric(colorField)) {
                color = colorField;
            } else if (Ext4.isString(colorField)) {
    ...
    constructor : function(config) {
        ...
        this.addEvents(
             ...
             * @param {Ext4.data.Record} record the record associated to the point
        ...
        Ext4.applyIf(config,{
            events:{
                click: Ext4.bind(this.onPointClick, this)
            }
        });
    ...
    buildInitData:function(items, data) {
        ...
        record = items[0];
        if (this.dataIndex && record && Ext4.isArray(record.data[this.dataIndex])) {
        ...
        var xAxis = (Ext4.isArray(chartConfig.xAxis)) ? chartConfig.xAxis[0] : chartConfig.xAxis;
