 ...
 *         store: Ext4.StoreMgr.get('PopDensity1'),
 *
 *         // Get the mapData from store instead
 *         mapDataStore: Ext4.StoreMgr.get('MapData2'),
 ...
 *     }]
 */
Ext4.define('Chart.ux.Highcharts.MapSerie', {
     ...
     *          store: Ext4.StoreMgr.get('USCountiesData'),
     *          dataNameField: 'name',
     *          dataValueField: 'value',
     *
     *          mapDataPathField: 'path',
     *          mapDataStore : Ext4.StoreMgr.get('USCountiesMap'),
    ...
    setMap: function(highmaps) {
        ...
        if (this.updateMap && this.store) {
            var handler = this.store.on('load', function(store) {
                ...
                Ext4.each(highmaps.chart.series, function(series) {
    ...
    getMapSeriesData: function() {
        ...
        Ext4.isArray(items) && console.log("Call getMapSeriesData. Size " + items.length);
        Ext4.each(items, function(record, index) {
    ...
    createMapSeries: function() {
        ...
        Ext4.applyIf(seriesOpts, this.serie_config);
