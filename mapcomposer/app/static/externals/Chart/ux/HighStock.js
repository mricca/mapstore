...
Ext4.define("Chart.ux.HighStock", {
  ...
  initComponent : function() {
    ...
    if(this.stores.length) {
      for(var i = 0; i < this.stores.length; i++) {
        this.stores[i] = Ext4.data.StoreManager.lookup(this.stores[i]);
      }
    }
  ...
  afterRender : function() {
    ...
    if(this.stores) {
      Ext4.each(this.stores, function(store, index) {
    ...
    // Use Ext4.apply insteadof Ext4.applyIf
    Ext4.apply(this.chartConfig.chart, {
  ...
  onDataChange : function(store, eOpts) {
    // Don't use onDataChange for Ext4Js 4.0 because this is ambigous
    // Will be fixed in Ext4Js 4.1
  ...
  loadStores : function() {
    Ext4.each(this.stores, function(store) {
      store.load();
    });
  },
  destroy : function() {
