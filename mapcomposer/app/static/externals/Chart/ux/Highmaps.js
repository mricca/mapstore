 ...
 *
 *     var map = Ext4.Create('Chart.ux.Highmaps', {
 *         chartConfig {
 *              ....
 *         },
 *         series : [{
 *             // Default series type is 'map', no need to specify
 *             store: Ext4.StoreMgr.get('PopulationDensity'),
 ...
 *     }],
 *     drilldown: {
 *         store: Ext4.StoreMgr.get('Drilldown'),
 *
 ...
 * 
 *     series : [{
 *         store: Ext4.StoreMgr.get('USStates'),
 *         dataPathField: 'path',
 *         dataNameField: 'name',
 ...
 *                             var popup = Ext4.create("Ext4.menu.Menu", {
 ...
 *                                         Ext4.create('Demo.view.SetValue', {
 *                                             pointObj: this
 *                                         }).show();
 *                                     }
 *                                 }, {
...
Ext4.define("Chart.ux.Highmaps", {
     ...
     *          dataPathField: 'path',
     *          color: "silver",
     *          store: Ext4.StoreMgr.get('Borders'),
     *          tooltip: {
     *              pointFormat: "Border"
     *          }
     *      }, {
     ...
     *              radius: 3
     *          },
     *          store: Ext4.StoreMgr.get('Cities')
     *      }, {
     *          ....
     */
    series:[],
...
*/
	...
	Ext4.each(_this.series, function(s) {
	...
	if (dd && dd.store) {
		...
		callback: function(records) {
		    ...
		    Ext4.Object.each(groups, function(key, value) {
	...
	var config = Ext4.apply({}, _this.chartConfig);
	...
	var drilldown = null;
	if (_this.drilldown) {
	    drilldown = Ext4.applyIf({ store: null,
				      series: _this.ddData },
				    _this.drilldown);
	}
	...
	Ext4.each(this.handlers, function(hConf) {
	    Ext4.destroy(hConf.handler);
	});
    },
