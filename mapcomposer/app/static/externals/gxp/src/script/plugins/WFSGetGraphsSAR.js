 ...
 */
gxp.plugins.WFSGetGraphsSAR = Ext.extend(gxp.plugins.Tool, {

    /** api: ptype = gxp_wmsgetfeatureinfo */
    ptype: "gxp_wfsgetgraphssar",
    ...
    constructor: function(config) {
        gxp.plugins.WFSGetGraphsSAR.superclass.constructor.apply(this, arguments);
    },
    ...
    init: function(target) {
		gxp.plugins.WFSGetGraphsSAR.superclass.init.apply(this, arguments);
    ...
    addActions: function() {
        ...
        this.popupCache = {};
        var actions = gxp.plugins.WFSGetGraphsSAR.superclass.addActions.call(this, [{
            ...
            callback: function(response) {
                ...
                if(response.features.length > 0) {
                    if(displayGraph){
                        ...
                        if (campo.length === 0){
                            var data = "[";
                            for (var key in response.features[0].data) {
                                if (response.features[0].data.hasOwnProperty(key)) {
                                    if (key.charAt(0) === "d"){
                                        var dataText = key.substring(1, 9);
                                        var time = Date.parseDate(dataText, "Ymd");
                                        var cccc = new Date(time);
                                        data += "[" + cccc.getTime() + "," + response.features[0].data[key] + "],";
...
Ext.preg(gxp.plugins.WFSGetGraphsSAR.prototype.ptype, gxp.plugins.WFSGetGraphsSAR);
